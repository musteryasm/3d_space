...
class Scene extends EventEmitter {
  constructor(
    ...
    hasControls = true,
    clearColor = 'black',
    backgroundImage = null // Add backgroundImage parameter
  ) {
    // Since we extend EventEmitter, we need to instantiate it from here
    super();
    ...
    const backgroundTexture = backgroundImage ? textureLoader.load(backgroundImage) : null;

    // Create a plane geometry that covers the entire scene
    const planeGeometry = new THREE.PlaneGeometry(1000, 1000, 1, 1);

    // Create a material using the texture
    const planeMaterial = new THREE.MeshBasicMaterial({ map: backgroundTexture, side: THREE.DoubleSide });

    // Create the mesh with the geometry and material
    this.backgroundPlane = new THREE.Mesh(planeGeometry, planeMaterial);
    this.backgroundPlane.position.set(0, 0, -500); // Set the position to be behind the scene
  ...
  render() {
    // Render the scene without clearing the depth buffer
